...
execute ChipExtData { keys = ks } op ch =
  case nibbles op of
    ...
    (0xE, _, 0x9, 0xE) -> ch { pc = pc ch + if (registers ch V.! secondNibble op) `elem` ks then 2 else 0 }
    (0xE, _, 0xA, 0x1) -> ch { pc = pc ch + if (registers ch V.! secondNibble op) `notElem` ks then 2 else 0 }
